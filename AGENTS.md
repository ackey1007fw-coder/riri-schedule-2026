...
- Codex / Work / Astra では、通常作業でサブエージェント（delegated worker / `spawn_agent`）を自動起動しない。依頼者が明示した場合、または独立性の高い調査・レビューで明確な品質上の利益がある場合だけ使う。
- サブエージェントを使う場合も最小数にする。原則1、同時最大2とし、同じ目的で新しい子を増やす前に既存スレッド・既存エージェントを再利用する。
- 子へ渡すコンテキストは必要最小限にする。`fork_turns` を指定できる環境では、原則 `none` または必要最小限の直近turnに限定し、全履歴のforkを既定にしない。
- 子エージェントがさらに子を起動する再帰委任は、依頼者の明示指示がない限り行わない。
- Claude / Codex / ChatGPT などの役割分担は、別AI・別作業レーンの運用であり、自動サブエージェント起動の指示ではない。
- GitHub上の既存PRレビュー、現在のスレッド、既存エージェントで十分なら、新しいサブエージェントを起動しない。速度よりも、必要十分な品質を保ったうえでのトークン・使用枠効率を優先する。
